...
class TestDataFrameMixins(unittest.TestCase):

    def setUp(self):
        class TestDataFrame(unittest.TestCase, mixins.DataFrameMixins): pass  # noqa: E701

        setattr(self, 'kls', TestDataFrame())
        setattr(self, 'df1', pd.DataFrame({'foo': [1, 2, 3, 4, 5],
                                           'bar': ['a', 'b', 'c', 'd', 'e']}))
        setattr(self, 'df2', pd.DataFrame({'foo': [1, 2, 3, 4, 5],
                                           'bar': ['a', 'b', 'c', 'd', 'e']}))
        setattr(self, 'df3', pd.DataFrame({'foo': [1, 2, 3, 4, 5],
                                           'bar': ['1', '2', '3', '4', '5']}))
        setattr(self, 'schema', {'foo': int, 'bar': str})
    ...
    def test_type_checking(self):
        '''Is an AssertionError raised if arguments are not DataFrames?'''
        typed_asserts = [
            self.kls.assertDataFrameEqual,
            self.kls.assertDataFrameNotEqual,
            self.kls.assertDataFrameContainsSubset,
        ]
        msg = '%s argument is not a DataFrame'
        ...
        # DataFrame schema asserts take a DataFrame and a dict
    ...
    def test_merge(self):
        '''Merge DataFrames that already have a '_merge' column?'''
        self.assertEqual(self.kls._indicator(self.df1, self.df2), '_merge')

        self.df1['_merge'] = 'foo'
        self.assertNotEqual(self.kls._indicator(self.df1, self.df2), '_merge')
        del self.df1['_merge']

    def test_assert_dataframe_equality(self):
        self.kls.assertDataFrameEqual(self.df1, self.df2)
        self.kls.assertDataFrameNotEqual(self.df1, self.df3)

        msg = '%s != %s' % (self.df1, self.df3)
        with self.assertRaises(AssertionError) as e:
            self.kls.assertDataFrameEqual(self.df1, self.df3)
        self.assertEqual(e.exception.args[0], msg)

        msg = '%s == %s' % (self.df1, self.df2)
        with self.assertRaises(AssertionError) as e:
            self.kls.assertDataFrameNotEqual(self.df1, self.df2)
        self.assertEqual(e.exception.args[0], msg)

        # if DataFrames have different columns they're not equal
        msg = 'DataFrames do not have the same columns'
        with self.assertRaises(AssertionError) as e:
            self.df1.columns = ['bar', 'baz']
            self.kls.assertDataFrameEqual(self.df1, self.df2)
        self.assertTrue(e.exception.args[0].endswith(msg))

        # if DataFrames have different lengths they're not equal
        msg = 'DataFrames do not have the same number of rows'
        with self.assertRaises(AssertionError) as e:
            self.df1.loc[len(self.df1)] = {'foo': 1, 'bar': 'a'}
            self.kls.assertDataFrameEqual(self.df1, self.df2)
        self.assertTrue(e.exception.args[0].endswith(msg))

        msg = '%s == %s' % (self.df1.iloc[:-1], self.df2)
        self.kls.assertDataFrameNotEqual(self.df1, self.df2)
        with self.assertRaises(AssertionError) as e:
            self.kls.assertDataFrameNotEqual(self.df1.iloc[:-1], self.df2)
        self.assertEqual(e.exception.args[0], msg)

    def test_assert_dataframe_contains_subset(self):
        self.kls.assertDataFrameContainsSubset(self.df1, self.df2.iloc[:-1])
        with self.assertRaises(AssertionError):
            self.kls.assertDataFrameContainsSubset(self.df1, self.df3)

        # if subset has columns not in df it's not a subset
        self.df3['baz'] = self.df2['bar']
        with self.assertRaises(AssertionError) as e:
            self.kls.assertDataFrameContainsSubset(self.df1, self.df3)

        m = 'Columns %s ⊅ columns %s' % (self.df1.columns.values,
                                         self.df3.columns.values)
        self.assertEqual(e.exception.args[0].split(' : ')[-1], m)

        # if subset has more rows than df it's not a subset
        with self.assertRaises(AssertionError) as e:
            self.kls.assertDataFrameContainsSubset(self.df2.iloc[:-1], self.df1)

        m = 'Length %s < length %s' % (len(self.df1) - 1, len(self.df1))
        self.assertEqual(e.exception.args[0].split(' : ')[-1], m)

    def test_assert_dataframe_schema(self):
        self.kls.assertDataFrameSchema(self.df1, self.schema)

        msg = '%s column is not %s' % ('foo', float)
        with self.assertRaises(AssertionError) as e:
            self.kls.assertDataFrameSchema(self.df1, {'foo': float, 'bar': str})
        self.assertEqual(e.exception.args[0], msg)

        # missing columns
        msg = 'does not contain column baz'
        with self.assertRaises(AssertionError) as e:
            self.kls.assertDataFrameSchema(
                    self.df1, {'foo': int, 'bar': str, 'baz': float})
        self.assertTrue(e.exception.args[0].endswith(msg))

        # unexpected columns
        msg = 'contains unexpected column bar'
        with self.assertRaises(AssertionError) as e:
            self.kls.assertDataFrameSchema(self.df1, {'foo': int})
        self.assertTrue(e.exception.args[0].endswith(msg))

    def test_assert_dataframe_coerce_schema(self):
        self.kls.assertDataFrameCoerceSchema(self.df1, self.schema)

        msg = 'Cannot coerce %s column to %s' % ('foo', int)
        with self.assertRaises(AssertionError) as e:
            self.kls.assertDataFrameCoerceSchema(
                    pd.DataFrame({'foo': [1., 2., None]}), {'foo': int})
        self.assertTrue(e.exception.args[0].endswith(msg))
...
class TestPanelMixins(unittest.TestCase):

    def setUp(self):
        class TestPanel(unittest.TestCase, mixins.PanelMixins): pass  # noqa: E701

        setattr(self, 'kls', TestPanel())
        setattr(self, 'panel1', [1, 2, 3, 2, 1])
        setattr(self, 'panel2', [1, 2, 3])
        setattr(self, 'panel3', ['x', 'y', 'z'])
        setattr(self, 'panelsize', 3)
    ...
    def tearDown(self):
        ...
        delattr(self, 'panel2')
        delattr(self, 'panel3')
        delattr(self, 'panelsize')

    def test_type_checking(self):
        '''Is an AssertionError raised if ``panel`` is not sized/iterable?'''
        # assertions that require panels to be collections.Iterable
        typed_asserts = [
            self.kls.assertPanelMembersEqual,
            self.kls.assertPanelMembersNotEqual
        ]
        msg = '%s argument is not iterable'
    ...
    def test_assert_panel_members_equality(self):
        self.kls.assertPanelMembersEqual(self.panel1, self.panel2)

        msg = '%s panel != %s panel' % (self.panel1, self.panel3)
        with self.assertRaises(AssertionError) as e:
            self.kls.assertPanelMembersEqual(self.panel1, self.panel3)
        self.assertEqual(e.exception.args[0], msg)

        self.kls.assertPanelMembersNotEqual(self.panel1, self.panel3)

        msg = '%s panel == %s panel' % (self.panel1, self.panel2)
        with self.assertRaises(AssertionError) as e:
            self.kls.assertPanelMembersNotEqual(self.panel1, self.panel2)
        self.assertEqual(e.exception.args[0], msg)

        # unhashable types
        self.kls.assertPanelMembersEqual([set([1, 2, 3])],
                                         [set([1, 2, 3])])

        msg = '%s panel != %s panel' % ([set([1, 2, 3])],
                                        [set([1, 2, 3]), set([2, 3, 4])])
        with self.assertRaises(AssertionError) as e:
            self.kls.assertPanelMembersEqual([set([1, 2, 3])],
                                             [set([1, 2, 3]),
        ...
        self.kls.assertPanelMembersNotEqual([set([1, 2, 3])],
                                            [set([1, 2, 3]),
                                             set([2, 3, 4])])

        msg = '%s panel == %s panel' % ([set([1, 2, 3])], [set([1, 2, 3])])
        with self.assertRaises(AssertionError) as e:
            self.kls.assertPanelMembersNotEqual([set([1, 2, 3])],
                                                [set([1, 2, 3])])
        self.assertEqual(e.exception.args[0], msg)
    ...
    def test_trivial_asserts(self):
        assert_map = {
            self.kls.assertPanelMemberIn: 'assertIn',
            self.kls.assertPanelMemberNotIn: 'assertNotIn'
        }
